...
export default function EdiablePlantOils(props) {
  ...
  const handleCloseConfirm = () => {
    ...
    for (var i in listOfBilling){
      stockArr.push({id:listOfBilling[i]?.id,maxSize:2000,qty:listOfBilling[i]?.size,lastUpdatedBy:user.id})
      materialsArr.push({id:listOfBilling[i]?.id,price:listOfBilling[i]?.subTotal,qty:listOfBilling[i]?.size})
    ...
    axios.post('http://localhost:2000/warehouse/addstock', out)
        .then(response => {
