...
function createData(id, productlength, from, qty, status) {
  return {
    id,
    productlength,
    from,
...
export default function ProductionTransactionHistories() {
  ...
  const fun=async()=>{
   return await axios.get("http://localhost:2000/order/getdetailsbyid?id="+sessionStorage.getItem("id"))
  }
  React.useEffect(()=>{
    const aj=fun()
    aj.then((res)=>{
  ...
  React.useEffect(()=>{
    ...
    for (var i in arr){
      ...
      outArr.push(createData(arr[i]._id,arr[i].product.length,arr[i].createdBy,arr[i].product.length,arr[i].status))
    }
    setRows(outArr)
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        ...
                        padding="none"
                      >
                       <Link to="/singleHistory"
                      state={
                        {"id":orderId,"status":row.status}
                      } 
                      ...
                      <TableCell align="left">{row.status ==0?"Not Accepted":
                      steps[row.status-1]
